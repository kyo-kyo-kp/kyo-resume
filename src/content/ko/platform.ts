...
export const platform: Platform = {
  ...
  approach:
    '수집(배치·실시간) → 레이어별 가공 → API → 시각화까지를 하나의 플랫폼으로 운영하며 지표 정의를 마트 레이어에 고정했습니다. 그 위에서 재설계를 시작했습니다. 기준은 셋, 운영 단순성·구조적 통일성·전환 대응력입니다. 실행 도구보다 Raw Landing 계약(Manifest·Watermark·검증)을 먼저 고정하고, 오케스트레이션과 추출 실행을 분리했습니다.',
  change:
